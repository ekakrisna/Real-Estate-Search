<script type="text/x-template" id="license-tpl">
            ...
            <div class="row px-n2">
                ...
                <div class="px-2 col-sm-6 col-lg-auto" v-for="( option, index ) in options" :key="option.id" >
                    <div class="icheck-cyan">
                        <input type="radio" :checked="option.id == value" :value="option.id" :id="option.name" :name="option.name" @input="emitInput" :required="required" />
                        <label :for="option.name" class="mr-5">@{{ option.label }}</label>
                    </div>
                </div>

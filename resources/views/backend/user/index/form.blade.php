...
@section('top_buttons')
    @if ($page_type == "create")
        <a href="{{route('admin.company.user.index', $parent_company_id)}}" class="btn btn-info float-sm-right">@lang('label.list')</a>
    @else
        <a href="{{route('admin.company.user.create', $parent_company_id)}}" class="btn btn-info float-sm-right">@lang('label.createNew')</a>
        <a href="{{route('admin.company.user.index', $parent_company_id)}}" class="btn btn-info float-sm-right">@lang('label.list')</a>
    @endif
@endsection

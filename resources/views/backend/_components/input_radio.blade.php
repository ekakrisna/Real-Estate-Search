        ...
        <div class="field-group clearfix @error($name) is-invalid @enderror">
            ...
            @foreach($options as $option)
                ...
                <div class="icheck-cyan d-inline">
                    <input v-model="{{$name}}" type="radio" value="{{ $current_value }}" id="input-{{ $name }}-{{ $loop->index }}" name="{{ $name }}" {{ !empty($loop->first) && (!empty($value) || $value == "") ? "checked" : "" }} {{ $value == $current_value ? "checked" : "" }} />

@extends('frontend.lp.map.app')
@section('title', $title)
@section('description', '')
@section('page')
<div class="lp-map-page">
    ...
    <transition name="fade" mode="out-in">
        <template v-if="isTooFarToastShowed">
            <div class="lp-map-toast">
                <div class="lp-content-toast">
                    物件は縮尺が500m以下で表示されます。
                </div>
            </div>
        </template>
...
{{-- <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDIautqM0oUnD3MUiGP002uDC5xA63I3Es&libraries=places&region=JP&language=ja"></script> --}}
            ...
            methods: {
                ...
                getMarker: function(){
                    ...
                    if(this.map_zoom >= 14){
                        ...
                        const url =  @json(route('api.lp.propertylist'));
                    ...
                    if(markerData) {
                        ...
                        if(new_marker){
                            new_marker.map(item => {
                                ...
                                if (item.new) {
                                    var image = {
                                        url: '../../frontend/assets/images/bg_plot_area_new.png',
                                        scaledSize: new google.maps.Size(67, 74),
                                        labelOrigin: new google.maps.Point(33,49)
                                    };
                                }
                                else {
                                    var image = {
                                        url: '../../frontend/assets/images/bg_plot.png',
                                        scaledSize: new google.maps.Size(54, 51),
                                    };
                                }
                                // ------------------------------------------------------------------
                ...
                markerClick: function(marker){
                    ...
                    // property list URL
                    // --------------------------------------------------------------
                    const propertyUrl = 'https://ieuru.info/property';
                    // --------------------------------------------------------------
                    ...
                    // Search Condition
                    // --------------------------------------------------------------
                    const minPrice = this.filters.minimum_price < @json( toMan($lowest_price_filter) ) ? Number(@json(toMan($lowest_price_filter))) - @json(config('const.man_price_step')) : this.filters.minimum_price;
                    const maxPrice = this.filters.maximum_price > @json( toMan($highest_price_filter) ) ? Number(@json(toMan($highest_price_filter))) + @json(config('const.man_price_step')) : this.filters.maximum_price;
                    const minLandArea = this.filters.minimum_land_area < @json( toTsubo($lowest_land_area_filter) ) ? Number(@json(toTsubo($lowest_land_area_filter, 0))) - @json(config('const.tsubo_area_step')) : this.filters.minimum_land_area;
                    const maxLandArea = this.filters.maximum_land_area > @json( toTsubo($highest_land_area_filter) ) ? Number(@json(toTsubo($highest_land_area_filter, 0))) + @json(config('const.tsubo_area_step')) : this.filters.maximum_land_area;
